...
{
  public class HolidayProcessorBenchmark
  {
    ...
    public HolidayProcessorBenchmark(string file) : this()
    {
      _file = file;
    }

    public HolidayProcessorBenchmark(Stream stream) : this()
    {
      _stream = stream;
    }

    private HolidayProcessorBenchmark()
    {
      ...
      _pipe = new Pipe(new PipeOptions(null, null, null, 4096, 2048, 1024, true));
    }

    public async Task<string> Run()
    {
      _days[DayOfWeek.Monday] = 0;
      _days[DayOfWeek.Tuesday] = 0;
      _days[DayOfWeek.Wednesday] = 0;
      _days[DayOfWeek.Thursday] = 0;
      _days[DayOfWeek.Friday] = 0;

      Task readingTask;
      if (_file != null)
        readingTask = ReadFile(_pipe.Writer);
      else
        readingTask = ReadStream(_pipe.Writer, _stream);
      var processingTask = ProcessFile(_pipe.Reader);
    ...
    private async Task ReadFile(PipeWriter writer)
    {
      ...
      {
        await ReadStream(writer, file);
      }
    }

    private async Task ReadStream(PipeWriter writer, Stream stream)
    {
      while (true)
      {
        var memory = writer.GetMemory();
        int count = stream.Read(memory.Span);
        if (count == 0)
          break;
